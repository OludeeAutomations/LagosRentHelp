...
7. Run `supabase_performance_hardening_migration.sql` to add indexes for public search, landlord portfolios, leads, and administrator dashboards.
8. Redeploy the `verify-nin` Edge Function after the SQL migrations so new NIN checks save the verified identity name.
9. Sign out and sign back in with `info@lagosrenthelp.ng`.
10. Open `/admin/verifications` to review landlord applications.
11. Open `/admin/accounts` to grant admin access to other people. Each person must register and confirm a normal account before the primary administrator enters their email.
