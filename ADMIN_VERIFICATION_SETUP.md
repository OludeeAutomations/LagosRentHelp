...
6. Redeploy the `verify-nin` Edge Function after the SQL migrations so new NIN checks save the verified identity name.
7. Sign out and sign back in with `info@lagosrenthelp.ng`.
8. Open `/admin/verifications` to review landlord applications.
9. Open `/admin/accounts` to grant admin access to other people. Each person must register and confirm a normal account before the primary administrator enters their email.
