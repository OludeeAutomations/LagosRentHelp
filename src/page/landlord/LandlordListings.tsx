            ...
            <div className="divide-y">
              {listings.map((listing) => (
                <div key={listing._id} className="flex flex-col gap-4 py-5 sm:flex-row sm:items-center">
                  ...
                  <div className="min-w-0 flex-1"><h3 className="truncate font-semibold">{listing.title}</h3><p className="text-sm text-gray-500">{listing.location} · {formatPrice(listing.price)}</p><div className="mt-2 flex gap-2"><Badge variant="outline">{listing.approvalStatus || "pending"}</Badge><Badge variant="secondary">{listing.status}</Badge></div></div>
